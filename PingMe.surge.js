...
function saveStore(store) {
  store.write(JSON.stringify(store), storeKey);
}
...
function runAccount(acc, index, total) {
  ...
  function fetchApi(path, useFakeId) {
    ...
    const attempt = (n) => requestGet({ url: buildUrl(path, acc.capture, overrideId), method: 'GET', headers })
      .catch(err => {
